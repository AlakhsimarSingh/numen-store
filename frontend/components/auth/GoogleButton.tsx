...
export default function GoogleButton({
  ...
  function handleClick() {
    // Full page navigation — this needs to leave the SPA entirely so the
    // browser can complete the real OAuth redirect dance with Google.
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className="flex w-full items-center justify-center gap-3 rounded-full border border-white/10 bg-bg px-5 py-3 font-body text-sm font-medium text-ink transition-colors hover:border-white/20"
    >
      <GoogleIcon />
      {label}
    </button>
